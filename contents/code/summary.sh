#!/usr/bin/env bash
# Copyright (C) 2026 gabrielsitta4 — GPL-3.0-or-later
...
# it: the group right above "plugin=com.gabrielsitta4.cctop" plus
...
widget_cfg() {
  ...
  awk -v key="$1" '
    /^\[/ { group = $0; next }
    $0 == "plugin=com.gabrielsitta4.cctop" { want = group "[Configuration][General]"; next }
